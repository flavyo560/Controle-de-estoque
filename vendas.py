...
def finalizar_venda(
    ...
    venda_id_existente: Optional[int] = None
) -> tuple[bool, str, Optional[int]]:
    """
    Finaliza uma venda com transação atômica.
    
    Se venda_id_existente for fornecido, atualiza a venda existente (útil para vendas em aberto).
    Caso contrário, cria uma nova venda.
    ...
    Args:
        ...
        venda_id_existente: ID de uma venda 'em_aberto' para ser finalizada (opcional)
    
    Returns:
        Tupla (sucesso, mensagem, venda_id)
    """
    from database import inserir_venda, registrar_movimentacao, supabase
    ...
    try:
        ...
        # 5. Inserir ou Atualizar venda
        if venda_id_existente:
            # Se a venda já existia como 'em_aberto', precisamos primeiro 
            # verificar se ela realmente existe e está aberta
            venda_atual = supabase.table('vendas').select('status').eq('id', venda_id_existente).execute()
            if not venda_atual.data or venda_atual.data[0]['status'] != 'em_aberto':
                return False, f"Venda {venda_id_existente} não encontrada ou não está em aberto.", None
            
            # Atualizar venda existente
            supabase.table('vendas').update(dados_venda).eq('id', venda_id_existente).execute()
            venda_id = venda_id_existente
            
            # Limpar itens e pagamentos antigos (serão reinseridos)
            ...
            if venda_id is None:
                return False, "Erro ao registrar venda no banco de dados.", None
        ...
        carrinho.limpar()
        return True, f"Venda finalizada com sucesso! ID: {venda_id}", venda_id
        
    except Exception as e:
        return False, f"Erro ao finalizar venda: {str(e)}", None


def salvar_venda_aberta(
    carrinho: Carrinho,
    usuario_id: int,
    cliente_id: Optional[int] = None,
    venda_id_existente: Optional[int] = None
    ...
    """
    Salva a venda atual como 'em_aberto' para continuação posterior.
    Não realiza baixa de estoque nem exige pagamentos.
    """
    from database import supabase
    ...
    try:
        ...
        if venda_id_existente:
            supabase.table('vendas').update(dados_venda).eq('id', venda_id_existente).execute()
            venda_id = venda_id_existente
            # Limpar itens para reinserir
            supabase.table('itens_venda').delete().eq('venda_id', venda_id).execute()
        else:
            response = supabase.table('vendas').insert(dados_venda).execute()
        ...
        return True, f"Venda #{venda_id} salva em aberto.", venda_id
    except Exception as e:
        return False, f"Erro ao salvar: {str(e)}", None


def buscar_vendas_abertas() -> List[dict]:
    """Retorna lista de vendas com status 'em_aberto'."""
    from database import supabase
    try:
        response = supabase.table('vendas').select("*, clientes(nome)").eq('status', 'em_aberto').order('created_at', desc=True).execute()
        return response.data or []
    except:
        return []


def carregar_venda_aberta(venda_id: int) -> tuple[Carrinho, Optional[int], Optional[int]]:
    """Carrega uma venda aberta para o carrinho."""
    from database import supabase
    
    response = supabase.table('vendas').select("*, itens_venda(*, produtos(*))").eq('id', venda_id).execute()
    if not response.data:
        return Carrinho(), None, None
    
    venda = response.data[0]
    novo_carrinho = Carrinho()
    novo_carrinho.desconto_percentual = float(venda.get('desconto_percentual', 0))
    novo_carrinho.desconto_valor = float(venda.get('desconto_valor', 0))
    
    for item in venda.get('itens_venda', []):
        prod = item.get('produtos', {})
        novo_item = ItemCarrinho(
            produto_id=item['produto_id'],
            descricao=prod.get('descricao', 'Produto não encontrado'),
            quantidade=item['quantidade'],
            preco_unitario=float(item['preco_unitario']),
            estoque_disponivel=int(prod.get('quantidade', 0))
        )
        novo_carrinho.itens.append(novo_item)
        
    return novo_carrinho, venda.get('cliente_id'), venda['id']
